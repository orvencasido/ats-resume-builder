...
export const AwardsForm: React.FC<Props> = ({ items, onChange }) => {
  ...
  const handleUpdateItem = (index: number, field: keyof AwardItem, value: string) => {
    const updated = [...items];
    updated[index] = {
      ...updated[index],
      [field]: value,
        ...
        >
          <div className="flex items-center justify-between border-b border-slate-100 pb-2">
            <span className="font-semibold text-slate-800 text-sm">
              Award #{index + 1}: {item.title || 'New Award'}
            </span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div>
              ...
              <input
                ...
                placeholder="e.g. Google or IEEE"
            ...
            <div className="sm:col-span-2">
              ...
              <input
                ...
                placeholder="e.g. Employee of the Year or Best Innovation Award"
                className="w-full px-3 py-2 text-sm bg-slate-50 border border-slate-300 rounded-lg focus:bg-white focus:ring-2 focus:ring-slate-900 focus:border-slate-900 transition-colors"
              />
            </div>
          </div>
